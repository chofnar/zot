...
func generateData(sampleSize int) map[godigest.Digest]string {
	dataMap := make(map[godigest.Digest]string, sampleSize)
	// nolint: gosec
	seededRand := rand.New(rand.NewSource(time.Now().UnixNano()))
...
func BenchmarkPutLocal(b *testing.B) {
	...
	cache, _ := storage.Create("boltdb", cache.BoltDBDriverParameters{dir, "cache_test", false}, log)
